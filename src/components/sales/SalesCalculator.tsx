...
import { 
  ...
  CreditCard, Banknote, Ship, Scale, DollarSign, User, Tag
} from 'lucide-react';
...
export default function SalesCalculator() {
  ...
  if (step === 1) {
    return (
      <div className="flex flex-col h-screen bg-neutral-950 text-white">
        <div className="bg-neutral-900 border-b border-neutral-800 px-4 py-3 sticky top-0 z-20 flex justify-between items-center shadow-lg">
            <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {batches.map((batch) => (
                <div key={batch.id} className="relative group">
                    <Button onClick={() => handleSelectBatch(batch)} className="w-full h-auto flex flex-col items-start justify-between p-4 bg-neutral-900/80 border border-neutral-800 hover:border-blue-500 hover:bg-neutral-900 text-left shadow-lg rounded-2xl transition-all duration-200 aspect-[4/3]" variant="ghost">
                        <div className="w-full">
                            <span className="block font-black text-xl text-white mb-1 truncate w-full">{batch.size}</span>
                            <span className="block text-xs font-semibold text-neutral-400">S/ {batch.price.toFixed(2)} <span className="font-normal opacity-60">/kg</span></span>
                        </div>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {recentSales.map((sale) => {
                        ...
                        return (
                            <div key={sale.id} className="bg-neutral-900 p-4 rounded-2xl border border-neutral-800 shadow-sm flex justify-between items-center relative overflow-hidden">
                                {sale.isPaid ? 
                                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500"></div> :
                                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-rose-500"></div>
                                }
                                <div className="flex-1 pl-2">
                                    {/* Header: Producto y Hora */}
                                    <div className="flex items-center justify-between pr-2 mb-1.5">
                                        <div className="flex items-center gap-2">
                                            <span className="font-black text-white text-base">{sale.batch.size}</span>
                                            {sale.customerName && !sale.isPaid && (
                                                <span className="text-[10px] bg-rose-900/30 text-rose-400 px-1.5 py-0.5 rounded flex items-center gap-1 border border-rose-900/50">
                                                    <User size={10} /> {sale.customerName}
                                                </span>
                                            )}
                                        </div>
                                        <span className="text-[10px] text-neutral-500 font-mono">{formatPeruTime(sale.createdAt)}</span>
                                    </div>

                                    {/* Detalles: Peso y Precio unitario */}
                                    <div className="flex items-center gap-3 text-xs text-neutral-400 mb-2">
                                        <span className="flex items-center gap-1"><Scale size={12}/> {Number(sale.weightKg).toFixed(2)} kg</span>
                                        <span className="flex items-center gap-1"><Tag size={12}/> S/ {Number(sale.pricePerKg).toFixed(2)}</span>
                                    </div>

                                    {/* Footer: Totales y Estado */}
                                    <div className="flex items-center gap-2">
                                        <div className="text-lg font-black text-white mr-2">
                                            S/ {Number(sale.totalPrice).toFixed(2)}
                                        </div>
                                        
                                        {sale.isPaid ? (
                                            <span className="text-[10px] font-bold text-emerald-400 bg-emerald-900/20 px-2 py-0.5 rounded border border-emerald-900/50 flex items-center gap-1">
                                                <Banknote size={10}/> Pagado
                                            </span>
                                        ) : (
                                            <span className="text-[10px] font-bold text-rose-400 bg-rose-900/20 px-2 py-0.5 rounded border border-rose-900/50 flex items-center gap-1">
                                                <CreditCard size={10}/> Debe: S/ {debt.toFixed(2)}
                                            </span>
                                        )}
                                        
                                        {sale.status === 'EN_PUESTO' && (
                                            <span className="text-[10px] font-bold text-amber-400 bg-amber-900/20 px-2 py-0.5 rounded border border-amber-900/50 flex items-center gap-1">
                                ...
                                {/* Botones de Acción */}
                                <div className="flex flex-col gap-2 pl-3 ml-1 border-l border-neutral-800">
                                    <button className="w-8 h-8 flex items-center justify-center text-blue-400 hover:text-white hover:bg-blue-600 rounded-lg transition-colors" onClick={() => handleEdit(sale)}>
                                        <Pencil size={16} />
                                    </button>
                                    <button className="w-8 h-8 flex items-center justify-center text-rose-400 hover:text-white hover:bg-rose-600 rounded-lg transition-colors" onClick={() => handleDelete(sale.id)}>
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
  ...
  // --- VISTA CALCULADORA (STEP 2 - SIN CAMBIOS SIGNIFICATIVOS VISUALES, SOLO FUNCIONALES) ---
  return (
    <div className={`flex flex-col h-screen max-h-screen bg-neutral-950 text-white`}>
      ...
      <div className="flex-1 p-4 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
            <Button onClick={toggleMode} variant="outline" className="h-14 bg-neutral-900 border-neutral-700 text-neutral-300 hover:bg-neutral-800 hover:text-white border-0 rounded-xl">
            ...
            <Button onClick={() => setIsPendingPickup(!isPendingPickup)} className={`h-14 border-0 rounded-xl ${isPendingPickup ? 'bg-amber-500 hover:bg-amber-600 text-black font-bold' : 'bg-neutral-900 border border-neutral-800 text-neutral-300 hover:bg-neutral-800'}`}>
        ...
        <Card className="p-4 bg-neutral-900 border-neutral-800 rounded-2xl">
            <div className="flex gap-2 mb-3">
                <button onClick={() => { setPaymentType('full'); setPartialAmount(''); }} className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all ${paymentType === 'full' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/50' : 'bg-neutral-950 border border-neutral-800 text-neutral-400'}`}>Pagado</button>
                <button onClick={() => { setPaymentType('partial'); }} className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all ${paymentType === 'partial' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-900/50' : 'bg-neutral-950 border border-neutral-800 text-neutral-400'}`}>A Cuenta</button>
                <button onClick={() => { setPaymentType('zero'); setPartialAmount(''); }} className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all ${paymentType === 'zero' ? 'bg-rose-600 text-white shadow-lg shadow-rose-900/50' : 'bg-neutral-950 border border-neutral-800 text-neutral-400'}`}>Fiado</button>
            </div>
            ...
            {(paymentType !== 'full') && (
                <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <input type="text" placeholder="Nombre del Cliente (Obligatorio)" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-sm text-white placeholder:text-neutral-600 outline-none focus:border-blue-500 transition-colors" />
